...
/*static*/ FILE* ZKCheckExecutor::exceptionOutput = stdout;

ZKCheckExecutor::ZKCheckExecutor()
...
bool ZKCheckExecutor::parseFromArgs(ZKCheck *cppcheck, int argc, const char* const argv[])
{
    ...
    if (success) {
        /*if (parser.GetShowVersion() && !parser.GetShowErrorMessages()) {
            const char * const extraVersion = zkcheck->extraVersion();
            if (*extraVersion != 0)
                std::cout << "ZKcheck " << zkcheck->version() << " ("
                          << extraVersion << ')' << std::endl;
            else
                std::cout << "ZKcheck " << zkcheck->version() << std::endl;
        }*/
...
int ZKCheckExecutor::check(int argc, const char* const argv[])
{
    Preprocessor::missingIncludeFlag = false;
    Preprocessor::missingSystemIncludeFlag = false;

    ZKCheck zkCheck(*this, true);

    const Settings& settings = zkCheck.settings();
    _settings = &settings;

    if (!parseFromArgs(&zkCheck, argc, argv)) {
        return EXIT_FAILURE;
    }
    ...
    if (zkCheck.settings().exceptionHandling) {
        return check_wrapper(zkCheck, argc, argv);
    } else {
        return check_internal(zkCheck, argc, argv);
    }
}
 ...
 *
 */
int ZKCheckExecutor::check_wrapper(ZKCheck& zkcheck, int argc, const char* const argv[])
{
#ifdef USE_WINDOWS_SEH
    FILE *outputFile = stdout;
    __try {
        ...
        fputs("Please report this to the zkcheck developers!\n", outputFile);
        return -1;
    }
#endif
}
 ...
 * */
int ZKCheckExecutor::check_internal(ZKCheck& zkcheck, int /*argc*/, const char* const argv[])
{
    Settings& settings = zkcheck.settings();
    _settings = &settings;
    ...
    if (settings.jobs == 1) {
        ...
        for (std::map<std::string, std::size_t>::const_iterator i = _files.begin(); i != _files.end(); ++i) {
            if (!_settings->library.markupFile(i->first)
                ...
                returnValue += zkcheck.check(i->first);
    ...
    if (settings.isEnabled("information") || settings.checkConfiguration) {
        const bool enableUnusedFunctionCheck = zkcheck.unusedFunctionCheckIsEnabled();
    ...
    if (!settings.checkConfiguration) {
        zkcheck.tooManyConfigsError("",0U);
...
void ZKCheckExecutor::reportErr(const std::string &errmsg)
{
...
void ZKCheckExecutor::reportOut(const std::string &outmsg)
{
    std::cout << outmsg << std::endl;
}

void ZKCheckExecutor::reportProgress(const std::string &filename, const char stage[], const std::size_t value)
{
    (void)filename;
...
void ZKCheckExecutor::reportInfo(const ErrorLogger::ErrorMessage &msg)
{
    reportErr(msg);
}

void ZKCheckExecutor::reportStatus(std::size_t fileindex, std::size_t filecount, std::size_t sizedone, std::size_t sizetotal)
{
    if (filecount > 1) {
...
void ZKCheckExecutor::reportErr(const ErrorLogger::ErrorMessage &msg)
{
    if (errorlist) {
...
void ZKCheckExecutor::setExceptionOutput(FILE* exception_output)
{
    exceptionOutput=exception_output;
}

FILE* ZKCheckExecutor::getExceptionOutput()
{
    return exceptionOutput;
}

bool ZKCheckExecutor::tryLoadLibrary(Library& destination, const char* basepath, const char* filename)
{
